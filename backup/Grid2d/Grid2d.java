...
public class Grid2d 
{
	...
	public List<Coordinate> findPath(int xStart, int yStart, int xGoal, int yGoal) 
        {
            Coordinate coord;
            List<Coordinate> coordinates = null;
            List<MapNode> nodes;
            nodes = SimplePathFinding.doAStar(new MapNode(xStart, yStart), new MapNode(
				xGoal, yGoal));
            if (nodes != null)
            {
                coord = new Coordinate(1.0,1.);
                coordinates = (List<Coordinate>) coord;
            }
            return(coordinates);
	}
}